...
(function(){
  ...
  // Localize a user-visible UI string. The AI's answer prose stays in English
  // (the system prompt says so); only the surrounding chrome is translated.
  var L = function(en, pt){ return (document.documentElement.lang === 'pt-BR') ? pt : en; };
  ...
  var SYSTEM_PROMPT=[
    ...
    'CRIMENET: 4,504 criminal organizations and 10,935 relationships from Wikipedia.',
    // <!-- crimenet-stats:end -->
  ...
  async function runAgent(userQuestion, onStatus){
    ...
    function renderSourcePill(url){
      return '<div class="ev-srcs"><span class="ev-srcs-label">'+L('Source:','Fonte:')+'</span> <a href="'+esc(url)+'" target="_blank" rel="noopener nofollow">'+esc(titleFromUrl(url))+'</a></div>';
    }

    function renderTimeToggle(timeVal){
      var tid='evt-'+Math.random().toString(36).slice(2,10);
      var btn='<button type="button" class="ev-toggle t" data-target="'+tid+'"><span>'+L('Time','Período')+'</span><span class="chev">▾</span></button>';
      var reveal='<div class="fp-reveal" id="'+tid+'" hidden><span class="ev-time">'+esc(timeVal||L('Unknown','Desconhecido'))+'</span></div>';
      return btn+reveal;
    }

    function renderQuoteToggle(quote){
      var qid='evq-'+Math.random().toString(36).slice(2,10);
      var btn='<button type="button" class="ev-toggle q" data-target="'+qid+'"><span>'+L('Quote','Citação')+'</span><span class="chev">▾</span></button>';
    ...
    function renderEdgeBlock(e){
      ...
      html+='<div class="rp-linkage-desc">'+(desc?esc(desc):'<em style="color:var(--text-muted)">'+L('No description.','Sem descrição.')+'</em>')+'</div>';
    ...
    function renderEvidenceSection(){
      ...
      pairKeys.forEach(function(pk){
        ...
        if(pair.summary){
          html+='<div class="rs-heading">'+L('AI Analysis','Análise da IA')+'</div>';
          html+='<div class="rs-ai-note">'+L('This narrative was generated by AI based on the evidence edges listed below. It is a summary, not a primary source.','Este texto foi gerado por IA com base nas evidências listadas abaixo. É um resumo, não uma fonte primária.')+'</div>';
          html+='<div class="rs-summary">'+esc(pair.summary)+'</div>';
          html+='<div class="rs-evidence-divider">'+L('Verified Evidence','Evidência Verificada')+'</div>';
        }
      ...
      if(hasPaths){
        collectedEvidence.paths.forEach(function(pathResult,pi){
          ...
          pathResult.paths.forEach(function(path,pp){
            ...
            html+='<div class="rp-section-title collapsed" onclick="var b=this.nextElementSibling;this.classList.toggle(\'collapsed\');b.classList.toggle(\'collapsed\');">'+L('Path','Caminho')+' '+(pp+1)+'<span class="rp-section-count">'+hops+' '+L('hop','salto')+(hops!==1?'s':'')+'</span></div>';
            html+='<div class="rp-section-body collapsed">';
      ...
      if(hasRoutes){
        collectedEvidence.routes.forEach(function(routeResult,ri){
          ...
          routeResult.routes.forEach(function(route,rp){
            ...
            html+='<div class="rp-section-title collapsed" onclick="var b=this.nextElementSibling;this.classList.toggle(\'collapsed\');b.classList.toggle(\'collapsed\');">'+L('Cooperation Route','Rota de Cooperação')+' '+(rp+1)+'<span class="rp-section-count">'+route.hops+' '+L('hop','salto')+(route.hops!==1?'s':'')+'</span></div>';
            html+='<div class="rp-section-body collapsed">';
    ...
    for(var iter=0;iter<MAX_ITERATIONS;iter++){
      if(onStatus)onStatus(iter===0?L('Reasoning…','Raciocinando…'):L('Looking up data…','Consultando dados…'));
      ...
      if(!resp.ok)throw new Error(L('API error: HTTP ','Erro da API: HTTP ')+resp.status);
      var data=await resp.json();
      if(data.error){
        var em=data.error.message||data.error;
        if(em&&em.indexOf('rate')>=0)throw new Error(L('Rate limited. Wait a moment and try again.','Limite de requisições atingido. Aguarde um momento e tente novamente.'));
        throw new Error(L('API error','Erro da API'));
      }
      var choice=data.choices&&data.choices[0];
      if(!choice)throw new Error(L('No response from API','Sem resposta da API'));
      ...
      if(msg.tool_calls&&msg.tool_calls.length>0){
        ...
        for(var i=0;i<msg.tool_calls.length;i++){
          ...
          if(onStatus)onStatus(L('Looking up:','Consultando:')+' '+toolName.replace(/_/g,' ')+'…');
      ...
      var answer=stripHtmlTags(msg.content||L('No response generated.','Nenhuma resposta gerada.'));
      var evidHtml=renderEvidenceSection();
      var urls=Object.keys(collectedSources);
      var sourcesHtml='';
      if(urls.length>0){
        sourcesHtml+='<div class="ask-src-head">'+L('Sources','Fontes')+'</div>';
    ...
    return {markdown:L('I ran into a loop trying to answer your question. Please try rephrasing it.','Entrei em um loop ao tentar responder sua pergunta. Tente reformulá-la.'),evidenceHtml:'',sourcesHtml:''};
  }
  ...
  function initAsk(){
    ...
    function newTurn(question){
      ...
      turn.innerHTML =
        '<div class="ask-msg ask-user"><div class="ask-msg-label">'+L('You','Você')+'</div><div class="ask-msg-body">'+esc(question)+'</div></div>'+
        '<div class="ask-msg ask-assistant"><div class="ask-msg-label">CRIMENET AI</div><div class="ask-msg-body"><span class="ask-spinner"></span> '+L('Thinking…','Pensando…')+'</div></div>';
    ...
    function submitQuestion(q, isFollowUp){
      ...
      runAgent(q, updateStatus).then(function(result){
        ...
        setAnswer(L('Error: ','Erro: ')+esc(err.message||L('Failed to reach the API.','Falha ao conectar à API.')));
      }).then(function(){
    ...
    if(examplesWrap){
      ...
      for(var g = 0; g < groups.length; g++){
        ...
        moreBtn.textContent = (document.documentElement.lang === 'pt-BR') ? ('+ ' + (btns.length - 2) + ' mais') : ('+ ' + (btns.length - 2) + ' more');
        (function(btn, total){
          btn.addEventListener('click', function(e){
            ...
            this.textContent = expanded ? (pt ? ('+ ' + (total - 2) + ' mais') : ('+ ' + (total - 2) + ' more')) : (pt ? 'Mostrar menos' : 'Show less');
          });
        })(moreBtn, btns.length);
