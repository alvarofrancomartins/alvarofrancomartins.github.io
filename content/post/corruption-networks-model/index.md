...
subtitle: Interact with networks that simulate real corruption networks

summary: Interact with networks that simulate real corruption networks
...
image:
  placement: 3
  caption: "A network simulated using the model of political corruption networks." 
  focal_point: "Smart"
  preview_only: false
---

{{% callout note %}}
I am pleased to finally publish one of the most important results I obtained during my master's degree. This work, however, would not have been possible without the help of my Ph.D. advisor, Prof. Dr. Haroldo Valentin Ribeiro. I also thank the other coauthors (Bruno R. da Cunha, Quentin S. Hanley, Sebastián Gonçalves and Matjaž Perc) for their collaborations to this work.
{{% /callout %}}

<br>

In our latest paper, [Universality of political corruption networks](/publication/brazilian-corruption-network), the findings of several empirical universalities in political corruption networks allowed us to propose a model to simulate the growth of corruption networks. Among other results[^1], our research revealed the prominent role of recidivist agents on corruption networks. For instance, we discover that recidivists act as bridges among minor corrupt groups and possibly coordinate peripheral criminals to perform more extensive and often much more harmful corruption processes. 

[^1]: I wrote a [post](https://www.alvarofrancomartins.com/post/corruption-networks) highlighting our main findings on political corruption networks. 

<br>

Motivated by these insights, we have used the recidivism rate (i.e, the fraction of corrupt agents recurring in the criminal activity), as the main parameter of the model. The simulations not only produce corruption networks with features very similar to those observed in the empirical data, but also indicates that corruption networks operate around a critical recidivism rate. Corruption networks simulated below this critical recidivism rate are completely fragmented, while networks generated above this critical value become overly connected. 

<br>

In this post, I will describe the model, display an interactive demonstration and present the model code in Python. 
...
We have proposed the model based on two of our main findings about political corruption networks. In particular, we have found that the size distribution of scandals is roughly approximated by an exponential distribution with a characteristic number of people around seven people for both countries. This result shows that political corruption runs in small groups that rarely exceed more than ten people (only 20% and 17% of corruption cases in Spain and Brazil, respectively). The surprising similarity in size distributions of scandals in both countries indicate a possible universal pattern related to political corruption processes.
...
The application below allows you to interact with the network's simulations[^2] and see how they evolve as you add new scandals. To play with it, first click in the **Start new network** button. Then, add as many scandals as you want using the **Add new scandal** button. Please note that, initially, only complete graphs will appear. After a while[^3], however, the recidivists will show up and then connect different parts of the network. I also prepared a [video](https://www.youtube.com/watch?v=ysGfMRU0gQg) demonstration of one possible simulation.
...
For more comparisons (both visual and quantitative), take a look at my other [post](/post/corruption-networks) where I show the results comparing the simulated and empirical (Brazilian and Spanish) corruption networks.
...
The most important aspect of this model is that it highlights the importance of recidivists in criminal contexts. Recidivists are able to connect different parts of networks and function as brokers. In this sense, the recidivism rate plays an important role in the evolution, structure and dynamics of corruption networks. In other words, repeat offenders are the "glue" of criminal networks, sustaining organized crime across time and space.

<br>

<!-- After studying the political corruption networks of both Brazil and Spanish, we have concluded that several patterns and features only happen because of repeat offenders. The coalescence process, for instance, does not happen without repeat offenders; they are the nodes responsible for connecting different components of the network. The recidivism rate also controls whether the network is more or less densely connected. The Brazilian corruption network has a giant component with density equal 0.0689, almost three times the density of the Spanish largest connected component (0.0245). Furthermore, a higher recidivism rate can cause the network to be more compact. The giant component of the Brazilian corruption network has lower average path length (2.99) and lower diameter (7) than the Spanish largest component (5.11 and 11, respectively). -->

From a practical perspective, our model may have important implications for law enforcement and intelligence activities. In a scenario where we are able to estimate the number of recidivists, we could consequently estimate very relevant features regarding the evolution and structure of the underlying network. Moreover, our model can even be used as a benchmark for related corruption network investigations, such as link prediction or efficiency in network dismantling. 
...
1. [_A.F.M, B. R. Cunha, Quentin S. H, S. Gonçalves, M. Perc & Ribeiro, H. V. __Universality of political corruption networks.__ Scientific Reports 7, 100-200, DOI: 10.1093/comnet/cny002s (2022)._](https://www.google.com)

2. [_Ribeiro, H. V., Alves, L. G. A., Martins, A. F., Lenzi, E. K. & Perc, M. __The dynamical structure of political corruption networks.__ J. Complex Networks 6, 989–1003, DOI: 10.1093/comnet/cny002s (2018)._](/publication/brazilian-corruption-network)