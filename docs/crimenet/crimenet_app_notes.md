...
The graph is extracted from **1,418 Wikipedia articles** (the number shown in the app's stat cards and README). The pipeline fetches 1,491 articles (`data/articles.csv`, `data/txts/`), but some — concept articles like "Anti-money laundering", non-org subjects — yield no nodes. The 1,418 figure counts only articles that actually contributed data to the graph, computed by `build_static_pages.py` as unique article URLs across all nodes' `own_sources` and `mentioned_in`. This is the real source footprint.
...
|------|-----------|-------------|-------------|
| `get_organization` | `query` (string), `is_defunct` (optional boolean) | `compact.json` + `centrality.json` | Look up a SPECIFIC organization by its name or alias. Returns full metadata: standard_name, aliases, description, country, time_period, is_defunct, founded_year, dissolved_year, degree, profiled flag, network_rank (degree/betweenness/pagerank each with value and rank out of ~3,500 connected orgs — gives immediate comparative context without a separate API call), country_links (all footprints with country, context, quote, source_url, source_title), footprint_summary (precomputed breakdown: total count plus by_continent with count + sorted country names — always use its exact numbers, never count from country_links), and sources (own_sources with fallback to mentioned_in). Use for named orgs ("Tell me about the Sinaloa Cartel"). For a comprehensive profile, also call get_connections + get_community. Covered by `bestMatch()`: exact match → prefix match → substring match → alias match. Limited to 15 results by default, cap relaxed to 100 when `is_defunct` is set. |
...
| `get_community` | `organization` (optional string), `community_id` (optional integer), `keyword` (optional string) | `communities.json` | Four modes: (1) no arguments — list all communities (id, title, short_summary, size, top_hubs; no full summary, no members — compact for scanning). (2) `keyword` — filter the list to only communities whose member names, title, or summary contain the keyword (case-insensitive `fold()` match). Always use `keyword` when searching for communities containing a specific org name or term — never scan 224 member lists yourself, you will miss results. (3) `community_id` — get one community by numeric id with the full summary and full member list. (4) `organization` — find which community a specific org belongs to, with full summary and full member list. Member lists include `[unprofiled — no data]` markers on orgs with no profiled Wikipedia article — the LLM must not invent details about them. Always scan with mode (1) or (2) first, then drill into specific communities with mode (3). |
...
| `get_centrality` | `metric` (string: degree/betweenness/pagerank), `organization` (optional string), `limit` (optional, default 20, max 50), `country` (optional string), `min_conflicts` (optional integer), `max_cooperation` (optional integer) | `centrality.json` | Get network centrality rankings and per-org metrics. **degree** = raw connection count (popularity). **betweenness** = bridging importance — how often an org sits on shortest paths (gatekeeping power). **PageRank** = weighted importance (quality over quantity, connections to important orgs matter more). Use for "most important/powerful/influential/connected" questions. Can return global top-N, filter by country, or return all three metrics for a single org with ranks out of 3,521. Also supports relationship-profile filtering: `min_conflicts` returns only orgs with at least N conflict edges; `max_cooperation` caps cooperation edges (set to 0 to find orgs with only conflicts and no cooperation). Filters compose with `country` to narrow by both geography and relationship profile. |
...
- Use `get_organization` first when a specific named organization is mentioned, to verify it exists; use the `is_defunct` flag for "which [type] are defunct?" or "which orgs are still active?" questions. For "tell me about X" questions, batch `get_organization` + `get_connections` + `get_community` in one multi-tool call to build a full profile matching the dashboard detail panel. `get_organization` now returns `network_rank` (degree/betweenness/pagerank with values and ranks) inline — use this for individual org stats. But for superlative or comparative claims across orgs ("the most X", "the highest Y", "ranked #1"), call `get_centrality` to see the full ranking. When listing footprints by region, always use the exact counts from `footprint_summary` (total and per-continent) rather than counting from the `country_links` array.
...
- Use `get_centrality` for superlative or ranking questions ("most important / powerful / influential / connected", "highest X", "oldest", "#1 in Y"). Default to betweenness when the question is vague — it best captures structural importance. Combine with `find_by_country` to filter by country, or with `get_organization` to get details on top-ranked orgs. For relationship-profile questions ("which orgs have only conflicts", "orgs with at least 5 conflicts", "pure rivals"), use `min_conflicts` and `max_cooperation` to filter by relationship composition — these work on the full connected set of 3,521 orgs and compose with `country` for regional breakdowns. `get_organization` now returns `network_rank` for individual org lookups, but that shows one org's rank — if the question asks for a comparative ranking or superlative ("the most", "the highest"), call `get_centrality` to get the full ordering.
...
- `app/js/ask_ai.js` — all tool definitions, tool implementations (13 tools: get_organization,
  ...
  `get_community` annotates member names with `[unprofiled — no data]` in
  `community_id` and `organization` modes when the org's `profiled` flag in
  `compact.json` is false — the LLM sees the marker and knows the org has no
  description, country, or other data to draw on. List-all mode does not return
  members at all (keeping the response compact: ~68 KB vs ~108 KB with members).
  The `keyword` parameter filters communities in code via `fold()` (case-insensitive
  matching against member names, title, and short summary) so the LLM never has to
  scan 224 member lists.
