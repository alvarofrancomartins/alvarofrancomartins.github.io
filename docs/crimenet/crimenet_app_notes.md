...
| `get_community` | `organization` (optional string), `community_id` (optional integer) | `communities.json` | Three modes: (1) no arguments — list all communities (id, title, short_summary, size, top_hubs, members; no full summary). (2) `community_id` — get one community by numeric id with the full summary. (3) `organization` — find which community a specific org belongs to, with full summary. Always scan with mode (1) first, then drill into specific communities with mode (2). |
