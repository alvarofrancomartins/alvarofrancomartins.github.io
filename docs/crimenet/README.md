...
│   └── build_static_pages.py             # → app/index.html + sitemap.xml + static_pages.css
│
...
│   ├── static_pages.css         # generated by build/ (shared styles for all pages)
...
│   └── update_readme_stats.py   # Sync README's headline counts with crimenet.json (7_apply_corrections.py calls it)
│
├── notebooks/
...
page_hyperlinks.csv
  ...
  crimenet.json        (corrected dataset)
    ...
    build_static_pages.py            → app/index.html, app/sitemap.xml, app/static_pages.css
    │
    ▼
...
After writing the final dataset it runs `tools/update_readme_stats.py` (best-effort, never fails), which rewrites the headline counts in this README between the `crimenet-stats` markers.
...
- **`index.html`, `sitemap.xml`, `static_pages.css`** — the front end: a two-panel dashboard (organizations and countries toggles in the left panel, detail rendering in the right panel), plus a sitemap for search engines and a shared stylesheet. Clicking an org or country renders its full detail in-panel (no individual org/country pages are generated anymore). Pass `--base-url` so canonical / Open Graph / sitemap URLs are absolute (it defaults to the live site).
- **`crimenet.json`** — a copy of the full dataset that `build_static_pages.py` drops into `app/data/` so the bundled visualizations (`footprints.html`, `knowledge_graph.html`) can load it at runtime (they `fetch("data/crimenet.json")`). ~11 MB raw, gzipped on the wire. The map/graph themselves are hand-authored static pages that live in `app/` and are reachable from `index.html`'s header links; the build just keeps their data in sync.
