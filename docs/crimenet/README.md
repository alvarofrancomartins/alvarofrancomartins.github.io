...
<p align="center">
  <a href="https://www.alvarofrancomartins.com/crimenet">
    <img src="featured.gif" alt="CRIMENET visualization" width="1000">
  </a>
</p>

> **Note:** this README was written by an AI from the actual project code, pipeline scripts, build tools, audit framework, and app source files. It is kept in sync with the codebase by `tools/update_readme_stats.py` and manual review.

<!-- crimenet-stats:start — auto-generated by tools/update_readme_stats.py; do not edit by hand -->
The current build holds **4,504 organizations** and **10,935 relationships** (1,032 organizations are profiled from their own article; 3,472 are kept as mentions; 317 are flagged defunct), extracted from 1,418 Wikipedia articles across four languages. Of these organizations, **3,520 (78%)** are connected to at least one other and **984 (22%)** are isolated, with no relationship extracted. An isolated organization reflects the absence of a documented tie in the Wikipedia articles processed, not necessarily the absence of any real-world connection.
<!-- crimenet-stats:end -->

The article count (1,418) is the number of articles that actually contributed nodes to the graph. The pipeline fetches 1,491 articles (counted in `data/txts/` and `data/articles.csv`), but some — concept articles like "Anti-money laundering", non-org subjects, etc. — yield no nodes, so they are not part of the graph's source footprint. The 1,418 figure is computed automatically by `build_static_pages.py` from unique article URLs across all nodes' `own_sources` and `mentioned_in`.

- Browse live: <a href="https://www.alvarofrancomartins.com/crimenet" target="_blank">alvarofrancomartins.com/crimenet</a>

To my knowledge, CRIMENET is the most extensive catalog of criminal organizations ever assembled. Wikipedia's own <a href="https://en.wikipedia.org/wiki/List_of_criminal_enterprises,_gangs,_and_syndicates">list of criminal enterprises, gangs, and syndicates</a> covers a few hundred groups; a search for "all criminal organizations worldwide" returns the candid admission that "an exhaustive list of all criminal organizations globally does not exist." CRIMENET's nearly 5,000 organizations and nearly 11,000 relationships come closer than anything else, each one backed by a specific Wikipedia source. This was an accidental achievement: the goal was to build a knowledge graph of how criminal organizations relate to each other, but because the LLM pipeline reads 1,418 articles across four languages and extracts every organization mentioned in each one, it ended up capturing the vast majority of criminal organizations documented on English, Italian, Portuguese, and Spanish Wikipedia.

## TL;DR — What you can do with CRIMENET

CRIMENET lets you explore the global underworld through a structured knowledge graph you can query, browse, and visualize. The star of the show is **Ask CRIMENET AI**, a natural language interface that answers complex questions by querying the graph's topology in real time. You ask in plain English, the AI calls tools that search the data, and every answer cites the underlying Wikipedia sources so claims stay auditable.

**Organizations.** Ask about any group's history, aliases, activity period, and country footprint: *"Tell me about the Sinaloa Cartel"*, *"What are the main 'Ndrangheta clans?"*, *"Which motorcycle clubs are defunct?"*, *"What are the oldest criminal organizations still active?"*

**Relationships and networks.** Trace who is allied with whom, who is at war, and how any two groups are connected: *"What's the relationship between the Sinaloa Cartel and CJNG?"*, *"Who are the Sinaloa Cartel's closest allies?"*, *"How are Hells Angels and the 'Ndrangheta connected?"*, *"What criminal organizations does the Sinaloa Cartel have conflicts with?"*

**Countries and geography.** Map criminal footprints across the world: *"What criminal organizations operate in Brazil?"*, *"Which countries does the 'Ndrangheta have a footprint in?"*, *"Compare organized crime in Mexico and Colombia"*, *"Which criminal organizations operate in both Colombia and Venezuela?"*

**Paths and indirect links.** Find chains of cooperation or conflict spanning continents: *"Are the Yakuza and the Sicilian Mafia connected?"*, *"Who are the allies of allies of Mara Salvatrucha?"*, *"Does the Sinaloa Cartel cooperate with the Sicilian Mafia?"*, *"Is there cooperation between Comando Vermelho and Sinaloa Cartel?"*

**Network centrality.** Rank organizations by their structural importance: *"What is the most important criminal organization in the global network?"*, *"Which Italian mafia group is the most central?"*, *"Which Mexican cartels have the most network influence?"*, *"How does the Sinaloa Cartel rank in network importance, and how does it compare to the American Mafia?"*

**Communities and hidden structure.** Discover clusters, bridge organizations, and triadic signals that suggest undocumented alliances or rivalries: *"What community does the Sinaloa Cartel belong to?"*, *"What are the largest criminal communities?"*, *"Which organizations bridge the most communities?"*, *"Who might the 'Ndrangheta be secretly allied with, based on shared partners?"*, *"What potential rivalries does the Sinaloa Cartel have based on shared adversaries?"*

Beyond the AI, you can browse the graph directly: the **connection finder** shows every documented tie between any two organizations with verbatim evidence quotes, the **footprints map** renders country-to-country presence on a world map, and the **3D knowledge graph** lets you fly through the full network. Everything runs in the browser with no backend. Start at [alvarofrancomartins.com/crimenet](https://www.alvarofrancomartins.com/crimenet) and ask anything.

## How the app is built (static architecture)

The app is a fully static knowledge graph in the browser: no database, no backend. For
the detail on how that works, how the graph is queried in real time, how the connection
finder and Ask CRIMENET AI call the data, how the sharded data files are laid out, and
how to build and deploy, read the architecture reference at
[app/crimenet_app_notes.md](app/crimenet_app_notes.md). This README covers the data
pipeline and dataset; that file covers the app and the static approach behind it.
...
The rest of this README is the detail behind each phase: **Pipeline** (extract), **Audit & correct** (fix errors), **Build the app assets** (build), **Visualize** (what the outputs look like), and **Ask CRIMENET AI** (natural language queries against the graph).
